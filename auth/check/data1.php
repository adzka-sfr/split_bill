...
// check apakah ada trip id
$stmt = $conn->prepare("SELECT COUNT(*) AS total FROM t_trip WHERE id = ?");
...
if ($row['total'] == 0) {
    ...
    <div class="alert alert-danger" role="alert">
        <strong>Error!</strong> Trip ID not found. Please check and try again.
    </div>
    ';
    exit;
}
...
// get trip name
$stmt = $conn->prepare("SELECT c_name FROM t_trip WHERE id = ?");
$stmt->bind_param("s", $trip_id);
$stmt->execute();
$result = $stmt->get_result();
...
<div class="row">
    <div class="col-12">
        <div class="table-container position-relative overflow-hidden">
            ...
            <div class="watermark"><span><?= htmlspecialchars($trip_name) ?></span></div>
